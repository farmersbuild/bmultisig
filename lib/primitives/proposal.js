...
const Outpoint = require('bcash/lib/primitives/outpoint');
const TX = require('bcash/lib/primitives/tx');
...
const status = {
  ...
  FORCE: 5,     // transaction was rejected by the admin.
  UNLOCK: 6     // transaction was rejected by force unlock of coin.
};
...
const statusMessages = [
  ...
  'Proposal has been rejected manually.',
  'Proposal has been rejected by unlocked coin.'
];
...
class Proposal extends Struct {
  ...
  /**
   * Check if we have locked coin.
   * @param {Outpoint} outpoint
   * @returns {Boolean}
   */

  static async hasOutpoint(db, outpoint) {
    const coin = await db.get(layout.c.encode(outpoint.hash, outpoint.index));
   ...
   * @returns {Promise<Number[]>} - proposal IDs.
   */

  static getPendingProposalIDs(db) {
    return db.range({
      gte: layout.e.min(),
      lte: layout.e.max(),
      parse: key => toU32BE(key.slice(1))
    });
  }
