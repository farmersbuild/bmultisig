...
const {Struct} = bufio;
const wcommon = require('bcash/lib/wallet/common');
const HDPublicKey = require('bcash/lib/hd/public');
...
class Cosigner extends Struct {
  ...
  getJSON(showDetails, network) {
    ...
    if (showDetails) {
      json.token = this.token.toString('hex');
      json.tokenDepth = this.tokenDepth;
    }
  ...
  inject(cosigner) {
    ...
    this.key = common.cloneHDPublicKey(cosigner.key);
